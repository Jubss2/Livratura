...
import { Box, Typography, CircularProgress, Alert, Container, TextField, IconButton, Button, Snackbar, makeStyles } from '@mui/material';
    ...
    const handleSalvarCatalogo = async (colecao: Partial<Catalogo>) => {
        try {
            if (catalogoEditando) {
                ...
                console.log("Coleção enviada para criação:", colecao);
            ...
            <Box sx={{ height: '97vh', display: 'flex', }}>
                <Container>
                    ...
                    <Typography variant='h4' color='secondary' gutterBottom textAlign="left" sx={{ marginTop: '100px', marginBottom: '20px' }}>
                        Busque por uma coleção
                    </Typography>
                    <TextField onChange={handleChangePesquisa} label='Busque por um tema ou autor que te interesse ' fullWidth sx={{ marginBottom: '10px', color: 'secondary' }} />
                    ...
                    {carregando ? (
                        ...
                        {colecoes.map((colecao) => (
                         colecao.tema.toLowerCase().includes(pesquisa.toLowerCase()) &&
